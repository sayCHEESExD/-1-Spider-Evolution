import { VISIBLE_REMOTE_PLAYERS } from '@spider/shared';
 ...
 * nearest are DRAWN - their bodies and pets - so fifteen Spider-Men with
...
export class RemotePlayerManager {
  ...
  /** Every remote currently drawn, with its session id. */
  forEachVisible(visit: (player: RemotePlayer, sessionId: string) => void): void {
    for (const id of this.visible) {
      const player = this.players.get(id);
      if (player) visit(player, id);
    }
  }
