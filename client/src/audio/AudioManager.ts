...
const COOLDOWNS: Readonly<Record<SoundName, number>> = {
  // Under the 0.3s attack interval, so every real attack sounds and a spammed click cannot double one.
  web: 0.2,
  enemyDeath: 0.04,
  jump: 0.15,
  swing: 0.25,
...
export class AudioManager {
  ...
  play(name: SoundName, intensity = 1, delay = 0): void {
    ...
    switch (name) {
      case 'web':
        // THWIP: a quick falling chirp over a soft snap.
        this.blip(now, 'triangle', 2400, 900, 0.07, 0.16 * level);
        this.blip(now + 0.015, 'square', 1400, 500, 0.05, 0.05 * level);
        break;
      case 'enemyDeath':
        if (this.playSample('enemyDeath', now, 0.55 * level)) break;
      ...
      case 'jump':
        ...
        break;
      case 'swing':
        if (this.playSample('swing', now, 0.55 * level)) break;
        this.blip(now, 'sine', 900, 300, 0.25, 0.2);
        break;
      case 'refuse':
